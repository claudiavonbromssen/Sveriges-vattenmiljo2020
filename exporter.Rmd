...
codebook <- tibble::tribble(
                 ...
                 "Biovolym cyanobakterier totalt",       "Cyanobacteria (mm3/l)",
                          ...
                          "Lättillgänglig fosfor",                "PO4-P (µg/l)",
                               "Oorganiskt kväve",         "Oorganiskt N (µg/l)",
                           "Totalt organiskt kol",                  "TOC (mg/l)",
                                          "Kisel",                   "Si (µg/l)",
...
vaxtplankton_mannkendall %>%
  ...
  write_excel_csv("vaxtplankton_status.csv", 
                  na = "")
...
trends  %>%
  ...
  write_excel_csv("vaxtplankton_serier.csv",
                  na = "")
```
...
kemi_sjo_mannkendall %>%
  ...
  transmute(VattenID = `EU id`, 
            ...
            `p-värde` = p.value,
            `Medianförändring per år` = slope) %>%
  write_excel_csv("sjo_kemi_status.csv",
                  na = "")
```
...
kemi_sjo_trender %>%
 ...
 # ungroup %>%
  ...
  write_excel_csv("sjo_kemi_serier.csv", 
                  na = "")
```
...
vattendragkemi_mannkendall %>%
  ...
  transmute(VattenID = `EU id`, 
            ...
            `p-värde` = p.value,
            `Medianförändring per år` = slope) %>%
  write_excel_csv("vattendrag_vattenkemi_status.csv",
                  na = "")
```
...
gam_models_vattendrag_kemi %>%
  ...
  transmute(VattenID = `EU id`, 
            ...
            `Andel regionala data` = 0) %>% 
  filter(!is.na(VariabelID)) %>%
  write_excel_csv("vattendrag_vattenkemi_serier.csv", na = "")
```
...
stationer_flodmynningar <- read_delim("stationer_flodmynningar.csv",
                                      ",", 
                                      escape_double = FALSE, 
...
Belastningar_mannkendall_stationer %>%
  ...
  transmute(Stationsnamn = StnNamn, 
            ...
            `p-värde` = p.value,
            `Medianförändring per år` = slope) %>%
  write_excel_csv("flodmynningar_belastningar_status.csv",
                  na = "")
```
...
belastningar_fitted_models %>%
  ...
  filter(!is.na(VariabelID)) %>%
  write_excel_csv("flodmynningar_belastningar_serier.csv", 
                  na = "")
```
...
fisk_mannkendall %>%
  ...
  write_excel_csv("fisk_status.csv", na = "")
```
...
fisk_sjo_trends  %>%
  ...
  transmute(VattenID = `EU_CD`, 
            ...
            `Antal provtagningstillfällen` = n,
            `Andel regionala data` = 0) %>%
  write_excel_csv("fisk_serier.csv",
                  na = "")
```
...
fisk_vdr_mannkendall %>%
  ...
  write_excel_csv("fisk_vdr_status.csv", na = "")
```
...
fisk_vdr_trends %>%
  ...
  transmute(VattenID = `EU_CD`, 
            ...
            `Antal provtagningstillfällen` = n,
            `Andel regionala data` = 0 ) %>%
  write_excel_csv("fisk_vdr_serier.csv",
                  na = "")
```
...
### Serier
```{r}
sjo_serier <- read_csv("vaxtplankton_serier.csv") %>% 
  full_join(read_csv("sjo_kemi_serier.csv")) %>% 
  full_join(read_csv("fisk_serier.csv"))
write_excel_csv(sjo_serier, "sjo_serier.csv")
```

### Mann-Kendall

```{r}
sjo_status <- read_csv("vaxtplankton_status.csv") %>% 
  full_join(read_csv("sjo_kemi_status.csv")) %>% 
  full_join(read_csv("fisk_status.csv"))
write_excel_csv(sjo_status, "sjo_status.csv")
```
...
### Serier
```{r}
vattendrag_serier <- read_csv("flodmynningar_belastningar_serier.csv") %>% 
  full_join(read_csv("fisk_vdr_serier.csv"))%>%
  full_join(read_csv("vattendrag_vattenkemi_serier.csv"))
write_excel_csv(vattendrag_serier, "vattendrag_serier.csv")
```

### Mann-Kendall
```{r}
vattendrag_status <- read_csv("vattendrag_vattenkemi_status.csv") %>% 
  full_join(read_delim("fisk_vdr_status.csv", 
    "\t", escape_double = FALSE, locale = locale(encoding = "ISO-8859-1"), 
    trim_ws = TRUE)) # Något konstigt med teckenkodningen på vattendrag-fisk-status

write_excel_csv(vattendrag_status, "vattendrag_status.csv")
```
