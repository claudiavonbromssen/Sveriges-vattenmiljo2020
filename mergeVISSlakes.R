...
sjöar<-read.csv("C:\\Users\\claudia\\Documents\\Projekt\\Havsmiljöinstitutet\\Sveriges Vattenmiljö 2018\\Sötvatten\\Koordinatfiler\\sjöar med long lat1.csv")%>%
    mutate(Stationsnamn=case_when(Stationsnamn =="V. Skälsjön" ~"Västra Skälsjön",
                         TRUE ~as.character(Stationsnamn)))

centroid<-read.csv("C:\\Users\\claudia\\Documents\\Projekt\\Havsmiljöinstitutet\\Sveriges Vattenmiljö 2018\\Sötvatten\\Koordinatfiler\\centroiderVISS_med long lat.csv")

fisk<-read.csv("C:\\Users\\claudia\\Documents\\Projekt\\Havsmiljöinstitutet\\Sveriges Vattenmiljö 2018\\Sötvatten\\Fisk\\SverigesVattenmiljöFiskSjöNpueB&Narter.csv")
...
VISS<- centroid %>% select(X, Y, MS_CD, EU_CD, SJONAMN) %>% right_join(new, merge, by=c("EU_CD"= "EU.id"))%>%
  mutate(MS_CD.C3=case_when(EU_CD== "SE656612-164132"~ "WA17355956", #Årsjön
...
#MS_CD, MS_CD.C3
Koordinatfil<-VISS %>% mutate(long = ifelse(is.na(X.y), ifelse(is.na(X.x), WGS84_E,X.x), X.y), 
...
write.csv(Koordinatfil, file = "C:\\Users\\claudia\\Documents\\Projekt\\Havsmiljöinstitutet\\Sveriges Vattenmiljö 2018\\Sötvatten\\Koordinatfiler\\LakeHierarkies_utan_regioner.csv",row.names=FALSE, na="")
