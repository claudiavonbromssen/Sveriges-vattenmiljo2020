...
vattenkemi_data <- joined_dataset %>% 
  select(vattenkemi_sjo[,1:25] %>% colnames(), 
         `NO2+NO3-N (µg/l)`,
  ...
  gather(variable, value, 26:(ncol(.)-2)) %>% # 2 if season
...
kemi_sjo_trender <- vattenkemi_data %>% 
  #  filter(n_years >= 10) %>% 
 ...
 # select(-min, -max) %>% 
 # unnest(data) %>% 
  arrange(Stationsnamn, `Stationskoordinat N/X`, `Stationskoordinat E/Y`, variable, year#, season
          ) %>% 
#  nest(-Stationsnamn, -`Stationskoordinat N/X`, -`Stationskoordinat E/Y`, -variable) %>% 
...
kemi_sjo_mannkendall <- 
 ...
 # group_by(Stationsnamn, `Stationskoordinat N/X`, `Stationskoordinat E/Y`, variable, Provtagningsår) %>% 
  ...
  mutate(fit = future_map(data, possibly(~rkt(date = as.data.frame(.x)$year, 
                                         #     block = as.data.frame(.x)$season,
